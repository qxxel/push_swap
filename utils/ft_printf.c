...
/*   Updated: 2024/01/09 14:42:34 by agerbaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_what_type(char c, va_list *ap, int *nb_char)
{
	if (c == 'c')
		...
		ft_putpointer(va_arg(*ap, unsigned long long), nb_char, 1);
	else if (c == 'd')
		ft_putnbr_fd(va_arg(*ap, int), 1, nb_char);
	else if (c == 'i')
		ft_putnbr_fd(va_arg(*ap, int), 1, nb_char);
	else if (c == 'u')
		ft_putnbr_unsigned_fd(va_arg(*ap, unsigned int), 1, nb_char);
	else if (c == 'x')
		ft_puthexa(va_arg(*ap, unsigned int), "0123456789abcdef", nb_char);
	else if (c == 'X')
		ft_puthexa(va_arg(*ap, unsigned int), "0123456789ABCDEF", nb_char);
	else if (c == '%')
		ft_putchar_fd(c, 1, nb_char);
	else if (c == '\0')
		(*nb_char) = -1;
	else
		return (0);
